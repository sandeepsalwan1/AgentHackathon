...
export async function memoryListPayload(url: URL, clinicId: string) {
...
export async function createMemoryFromBody(
...
export async function correctMemoryFromBody(
  body: Record<string, unknown>,
  actor: Actor,
  clinicId: string
): Promise<MemoryActionResult> {
...
export async function deleteMemoryFromBody(
  body: Record<string, unknown>,
  clinicId: string
): Promise<MemoryActionResult> {
  const memoryBody = parseMemoryBody(body);
