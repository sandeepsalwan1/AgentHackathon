...
export async function POST(request: Request) {
  try {
    ...
    return executeVetAgentWorkflow({ agent: "internal", routeIntent: "daily_ops", input: auth.body, actor: auth.actor, request });
  } catch (error) {
    return dbError(error, { route: "agent.daily_ops" });
  }
}
