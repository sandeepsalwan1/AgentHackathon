...
async function profilePayload(profileId: string | null | undefined) {
  if (!profileId) return {};
  const profiles = await listRecipientProfiles({ includeInactive: true });
  return {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (actor.role === "veterinarian" && actor.profileId) {
      ...
      await setRecipientProfile({ ...existing, displayName: nextName }, actor);
    }
    ...
    return NextResponse.json(
      {
        ...
        rename,
        ...(await profilePayload(actor.profileId))
      },
