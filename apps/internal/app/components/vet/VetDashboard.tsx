...
import { PetOwnersList, type PetOwner } from "./PetOwnersList";
...
export function VetDashboard({ session, onLogout }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<"queue" | "clients">("queue");
  ...
  function handleSelectOwner(owner: PetOwner) {
  ...
  return (
    <div className="vetShell">
      ...
      <header className="vetHeader">
        ...
        <div className="vetHeaderRight">
          ...
          <button
            ...
            onClick={() => { setChatOpen((o) => !o); setSelectedOwner(null); }}
      ...
      {/* Tab bar */}
      <div className="vetTabBar">
        <button
          className={`vetTab${activeTab === "queue" ? " vetTab--active" : ""}`}
          onClick={() => { setActiveTab("queue"); setSelectedOwner(null); }}
          type="button"
        >
          Dashboard
        </button>
        <button
          className={`vetTab${activeTab === "clients" ? " vetTab--active" : ""}`}
          onClick={() => { setActiveTab("clients"); setSelectedOwner(null); }}
          type="button"
        >
          Pet Owners
        </button>
      ...
      {/* Content */}
      <div className={`vetContent${(chatOpen || selectedOwner) ? (chatExpanded ? " vetContent--withChat--wide" : " vetContent--withChat") : ""}`}>
        <div className="vetMain">
          ...
          {activeTab === "clients" && (
            <PetOwnersList
              ...
              selectedOwnerName={selectedOwner?.clientName ?? null}
            />
          )}
        </div>

        {/* Chat panel */}
        {selectedOwner && !chatOpen && (
          <PetOwnerDetail
            owner={selectedOwner}
