# Eepish — Complete Product Intent, Missing Context, and Final Agent-Centric Plan

This document is a revised, fuller implementation brief for Eepish. It is meant to be handed to another AI or engineer so they can understand the complete intent, missing context, exact priorities, constraints, architecture, and execution plan.
...
Eepish is an AI-native operating layer for veterinary hospitals first, and later for many kinds of local businesses. The point is not to build a toy chatbot. The point is to build a system that can take action based on context, chain reasoning and tools together, and solve real hospital problems end to end.
...
This means Eepish should **not** start by rebuilding task management from scratch. The existing task manager should be treated as a reusable core module that agents can read from and write to.
...
Eepish should be built from four core layers.
...
eepish dev down
eepish db migrate
eepish db seed --dataset vet-sandbox
eepish tenant create "Tri-City Veterinary Hospital" --slug tri-city
eepish provider enable mock --tenant tri-city
eepish workflow install default-vet --tenant tri-city
eepish agent test external --tenant tri-city --scenario booking
eepish agent test external --tenant tri-city --scenario checkin
eepish agent test internal --tenant tri-city --scenario records-transfer
eepish agent test internal --tenant tri-city --scenario pricing-review
eepish import csv appointments.csv --tenant tri-city
eepish import csv clients.csv --tenant tri-city
eepish integration connect avimark --tenant tri-city
eepish integration validate --tenant tri-city
eepish agent trace --run-id <id>
```
