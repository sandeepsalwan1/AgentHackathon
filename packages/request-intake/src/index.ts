...
type ClientRequestIntakeOptions = {
  hospitalName?: string;
...
const defaultLogger: Required<RequestIntakeLogger> = {
...
function loggerFor(logger?: RequestIntakeLogger): Required<RequestIntakeLogger> {
  return {
...
function rateLimited(key: string, maxTrackedClients: number) {
  const now = Date.now();
...
export async function handleClientRequestIntake(
  request: Request,
  options: ClientRequestIntakeOptions = {}
): Promise<ClientRequestIntakeResult> {
